...
export function FoldMetrics({ folds }: { folds: FoldResult[] }) {
  const averageSharpe = folds.reduce((sum, fold) => sum + fold.oos_sharpe, 0) / Math.max(folds.length, 1)
  return (
      ...
      <div className="mt-4 h-[220px]">
        <ResponsiveContainer>
          <BarChart data={folds}>
            ...
            <Bar dataKey="oos_sharpe" radius={[6, 6, 0, 0]}>
              {folds.map((fold) => (
                <Cell key={fold.fold_id} fill={fold.oos_sharpe >= 0 ? colors.green : colors.red} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
